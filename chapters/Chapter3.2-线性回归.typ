...
岭回归（Ridge regression）和拉索回归（ Lasso regression ）是带有正则化的线性回归.#index[岭回归]#index[拉索回归]

岭回归在线性回归代价函数上添加了$L_2$正则化项：#index[L2正则化]
$
...
拉索回归在线性回归代价函数上添加了$L_1$正则化项#index[L1正则化]（拉索回归没有通用的闭式解）:
$
...
=== 广义线性回归
#definition[广义线性回归][#index[广义线性回归]
  广义线性回归的基本形式：
  $
