...
$
#remark[
而这个其实就是我们常见的Sigmoid函数了.#index[Sigmoid函数]
]]
...
对于给定训练数据集$T={(x_i,y_i)|_(x_i in R^n, y_i in {0,1})}$,可以应用极大似然估计法估计模型参数#index[极大似然估计],从而得到逻辑斯蒂回归模型.设
$
...
对数似然函数为
#note[
  This is Cross Entropy.#index[交叉熵]
  $
  "CE" = - sum P(x_i) log Q(x=x_i)
  $
]
$
...
$
  P(Y=k|x)&=frac(exp(w_k dot x),1+sum_(k=1)^(K-1) exp(w_k dot x)),\ #h(5em) k=1,2,dots,K-1\
  P(Y=K|x)&=frac(1,1+sum_(k=1)^(K-1) exp(w_k dot x)),\ #h(5em) k=K
$
这里,$x in R^(n+1), w_k in R^(n+1)$

这个其实就是softmax函数.#index[Softmax]将各个输出节点的输出值范围映射到[0, 1],并且约束各个输出节点的输出值的和为1.
]
#remark[
...
#algorithm[逻辑斯蒂回归模型][
  #index[逻辑斯蒂回归模型] *输入*：训练数据集 $T={(x_1,y_1),(x_2,y_2),dots,(x_N,y_N)}$,其中 $x_i in R^n, y_i in {0,1}$；学习率 $alpha$；最大迭代次数 $K$ 或收敛阈值 $epsilon$.
...
#algorithm[最大熵模型][
  #index[最大熵模型] *输入*：训练数据集 $T={(x_1,y_1),(x_2,y_2),dots,(x_N,y_N)}$；特征函数集合 ${f_1, f_2, dots, f_n}$.
