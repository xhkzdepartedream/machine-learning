...
$
  "logic"(p)=log p/(1-p)
$
这样取值范围就变成$(-infinity,+infinity)$了，这里就引入最关键的假设：#note[
...
对于给定训练数据集$T={(x_i,y_i)|_(x_i in R^n, y_i in {0,1})}$，可以应用极大似然估计法估计模型参数，从而得到逻辑斯蒂回归模型。设
$
...
=== 最大熵模型
假设离散随机变量$X$的概率分布是$P(X)$，我们前面介绍过其熵是$H(P)=-sum_x P(x)log P(x)$且有不等式$0 lt.eq.slant H(P) lt.eq.slant log |X|$成立。$|X|$表示$X$的取值个数。当且仅当$X$满足均匀分布的时候，右边的等号成立，即$X$服从均匀分布时，熵最大。#note[
