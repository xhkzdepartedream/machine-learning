...
==== 线性可分支持向量机
考虑一个二分类问题,假设输入空间与特征空间为两个不同的空间,输入空间为欧式空间或离散集合,特征空间为欧式空间或希尔伯特空间.线性可分支持向量机、线性支持向量机假设这两个空间的元素一一对应,并将输入空间中的输入映射为特征空间中的特征向量.非线性支持向量机利用一个从输入空间到特征空间的非线性映射将输入映射为特征向量.所有输入都由输入空间转换到特征空间,支持向量机的学习是在特征空间进行的.#note[
...
$H_1$与$H_2$平行,并且没有实例点落在他们中间,在二者之间形成一个长带,分离超平面与他们平行且位于他们中央.*长带的宽度,即$H_1$与$H_2$之间的距离称为间隔(margin)*.间隔的长度为$2/norm(w)$, $H_1$和$H_2$称为间隔边界.#note[
