...
= 支持向量机
== 引言
支持向量机(support vector machine, SVM)是一种二类分类模型#index[支持向量机],它的基本模型是定义在特征空间上的*间隔最大*的线性分类器,间隔最大使他有别于感知机；支持向量机还包括核技巧,这使它成为实质上的非线性分类器.

支持向量机的学习策略就是*间隔最大化*#index[间隔最大化],可形式化为一个求解凸二次规划的问题,也等价于正则化的合页损失函数的最小化问题.支持向量机的学习算法是求解凸二次规划的最优化算法.
