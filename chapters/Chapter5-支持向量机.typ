...
= 支持向量机
== 引言
支持向量机(support vector machine, SVM)是一种二类分类模型#index[支持向量机],它的基本模型是定义在特征空间上的*间隔最大*#index[间隔最大化]的线性分类器,间隔最大使他有别于感知机；支持向量机还包括核技巧,这使它成为实质上的非线性分类器.
...
#note[当输入空间为欧式空间或离散集合、特征空间为希尔伯特空间时,核函数表示将输入从输入空间映射到特征空间得到的特征向量之间的内积.通过使用核函数可以学习非线性支持向量机,等价于隐式地在高维的特征空间中学习线性支持向量机.这样的方法称为核技巧.核方法是比支持向量机更为一般的机器学习方法.]

#remark[
  按照套路算出 $alpha^\*$，然后检查一下是否在约束范围之内，最后用alpha算出w和b。
]
硬间隔：支持向量只能严格落在间隔边界（虚线）上 。它们到分离超平面的几何间隔全部精确等于 1 。  

软间隔：由于松弛变量 #mi(`\xi_i`) 的存在，对应的支持向量（即 #mi(`\alpha_i^* > 0`) 的点）分布非常广泛 ：  
+ 若 #mi(`\alpha_i^* < C`)，则 #mi(`\xi_i = 0`)，点恰好落在间隔边界上 。  
+ 若 #mi(`\alpha_i^* = C, 0 < \xi_i < 1`)，点在间隔边界与分离超平面之间（分类正确但踩线） 。  
+ 若 #mi(`\alpha_i^* = C, \xi_i = 1`)，点恰好在分离超平面上 。  
+ 若 #mi(`\alpha_i^* = C, \xi_i > 1`)，点位于分离超平面误分的一侧 。
== 线性可分支持向量机与硬间隔最大化
...
+ *线性空间*：定义了加法和数乘,空间中的元素可以通过加法和数乘进行线性组合.
+ *度量空间*：一个定义了距离的空间我们把它称为度量空间,而距离的定义需要满足:
...
+ *内积空间*：在一个线性空间上定义了内积运算的空间.定义了内积才能定义角度,定义正则.内积运算需要满足：
  - 严格正定性：$(x,x) gt.eq.slant 0$,等号成立当且仅当$x=0$
...
#definition[几何间隔][#index[几何间隔]
  但选择超平面时,只有函数间隔还不够.因为只要成比例地改变$w$和$b$,超平面并不会改变,而函数间隔却变成了两倍,因而要对$w,b$进行规范化,这时函数间隔就变成了几何间隔(geometric margin). 
...
#note[
  直观理解：外层$max_(alpha)$寻找使拉格朗日函数最大的乘子（惩罚违反约束的情形）,内层$min_(w,b)$在给定乘子下最小化目标.若约束$h_i(x) gt.eq.slant 0$被违反,拉格朗日乘子$alpha_i$会使对应项趋于无穷大,迫使$min$在优化时回到可行域内.
]
$
  max_(bold(alpha)) min_(w,b) L(w,b,bold(alpha))
$
$
...
原始问题满足拉格朗日对偶条件,所以存在$w^*, alpha^*$,使$w^*$是原始问题的解,$alpha^*$是对偶问题的解.
...
综上所述,我们得到以下算法
#algorithm[线性可分支持向量机][
  #index[线性可分支持向量机] *输入：*训练数据集$T={(x_1,y_1),(x_2,y_2),dots,(x_N,y_N)}$,其中,$x_i in cal(X)=R^n, y_i in cal(Y)={-1,+1},i=1,2,dots,N$.
  ...
  *输出：*分离超平面和分类决策函数.
  ...
  2. 计算$w^*=sum_(i=1)^N alpha_i^* y_i x_i$
    ...
    选择$alpha^*$的一个满足条件$alpha^*_j>0$的分量$alpha_j^*$,计算
    $
      b^*=y_j-sum_(i=1)^N y_i alpha_i^* (x_i dot x_j)
    $
    或者用:$b^* = y_j - w^* dot x_j$亦可。
  3. 求得分离超平面
$
  sum_(i=1)^N alpha_i^* y_i (x dot x_i)+b^*=0
$
...
综上所述,我们得到以下算法
#algorithm[线性支持向量机][
  #index[线性支持向量机] *输入：*训练数据集$T={(x_1,y_1),(x_2,y_2),dots,(x_N,y_N)}$,其中,$x_i in cal(X)=R^n, y_i in cal(Y)={-1,+1},i=1,2,dots,N$.
...
$
称为合页损失函数(hinge loss function)#index[合页损失].  下标“+”表示以下取正值的函数：
$
  [z]_+=cases(
    z #h(1em)z>0,
...
#note[醍醐灌顶!]一般来说,如果能用$R^n$中的一个超曲面将(之前提到的)正负例正确分开,则称这个问题为非线性可分问题.
#remark[
...
#definition[
  ...
  $
  则称$K(x,z)$为核函数#index[核函数],$phi.alt(x)$为映射函数,式中的运算为内积.
...
#example[
  ...
  $
    (x dot z)^2=(x^((1))z^((1))+x^((2))z^((2)))^2=(x^((1))z^((1)))^2+2x^((1))z^((1))x^((2))z^((2))+(x^((2))z^((2)))^2
  $
  所以可以取映射
  $
...
$
  f(x)
  &="sign"(sum_(i=1)^(N_s) alpha_i^* y_i phi.alt(x_i)dot phi.alt(x)+b^*)\
...
#algorithm[非线性支持向量机学习算法][
  #index[非线性支持向量机学习算法] *输入：*训练数据集$T={(x_1,y_1),(x_2,y_2),dots,(x_N,y_N)}$,其中$x_i in cal(X)=R^2,y_i in cal(Y)={-1,+1},i=1,2,dots,N$
...
如何高效地实现支持向量机学习是一个重要的问题,现在我们学习序列最小最优化(sequential minimal optimization, SMO)算法#index[SMO算法],于1998年由Platt提出.
...
#algorithm[SMO算法][
  #index[SMO算法] *输入*：训练数据集$T={(x_1,y_1),(x_2,y_2),dots,(x_N,y_N)}$,其中,$x_i in cal(X)=R^n, y_i in cal(Y)={-1,+1},i=1,2,dots,N$,精度$epsilon$,惩罚参数$C$.
...
#proof[
  ...
  *必要性.*由于$K(x,z)$是$cal(X)times cal(X)$上的正定核,所以存在从$cal(X)$到希尔伯特空间$cal(H)$的映射$phi.alt$,使得
  ...
  $
  表明$K(x,z)$关于$x_1,x_2,dots,x_m$的Gram矩阵是半正定的.
