...
核心直觉：空间扩张速度远超数据填充速度.在高维情形下出现的数据样本稀疏、距离计算困难等问题,是机器学习方法共同面临的严重障碍,被称为维数灾难（维数爆炸）.#index[维数灾难]
...
主成分分析（principal componentanalysis,PCA）是一种常用的无监督学习方法#index[主成分分析],这一方法*利用正交变换把由线性相关变量表示的观测数据转换为少数几个由线性无关变量表示的数据*,线性无关的变量称为*主成分*.主成分的个数通常小于原始变量的个数,所以*主成分分析属于降维方法*.主成分分析主要用于发现数据中的基本结构,即数据中变量之间的关系,是数据分析的有力工具,也用于其他机器学习方法的前处理.
...
#note[用mitex的都是sb,可读性和gemini做一桌]
