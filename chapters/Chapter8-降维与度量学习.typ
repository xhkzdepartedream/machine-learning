...
空间扩张速度远超数据填充速度.在高维情形下出现的数据样本稀疏、距离计算困难等问题,是机器学习方法共同面临的严重障碍,被称为维数灾难（维数爆炸）.#index[维数灾难]

为了缓解维数灾难,需要把原始高维度属性转换为一个低维度子空间,也就是学一个embedding.

#definition("多维缩放（Multiple Dimensional Scaling, MDS）")[#index[多维缩放]
...
当然实际操作中选前$d'$大的特征值即可.
#algorithm("MDS")[
  #index[MDS] 输入$bold(D) in RR^(m times m),$元素是距离$d_(i j)$;输入低维度数目$d'$.
  + 计算$bold(B)$的每一个元素.注意到$
b_(i j)  = -1/2 (d_(i j)^2 - d_(i .)^2 - d_(j .)^2 + d_(. .)^2)
$,其中\ $d_(i .) = 1/m sum_(j=1)^m d_(i j)^2 , d_(. j) = 1/m sum_(i=1)^m d_(i j)^2, d_(. .) = 1/m^2 sum_(i=1)^m sum_(j=1)^m d_(i j)^2$
  + 获取$bold(B)$的特征值和特征向量,取$tilde(bold(Lambda)) eq.delta "diag"{lambda_1,dots,lambda_(d')}$为$d'$个最大特征值构成的对角矩阵,$tilde(bold(V))eq.delta [v_1,dots,v_(d')]$为对应的特征向量矩阵.
  + return $tilde(bold(V)) tilde(bold(Lambda))^(1/2)$.对一个对角矩阵开平方根,就是对对角线上的数字做平方根.
...
#index[主成分分析]
*利用正交变换把由线性相关变量表示的观测数据转换为少数几个由线性无关变量表示的数据*,线性无关的变量称为*主成分*.主成分的个数通常小于原始变量的个数,所以*主成分分析属于降维方法*.主成分分析主要用于发现数据中的基本结构,即数据中变量之间的关系。
...
#note[
值得注意的是,这里的中心化是对*特征中心化*.例如,有样本$X = [x_1,x_2,dots,x_n]^T$(注意transpose了,这意味着$x_i$是每个样本),那么这个中心化就*不是*对$x_i$做,而是对每个样本的第$i$个特征做(也就是,$X = [f_1,dots,f_n]$的$f_i$中心化). 
...
#index[样本主成分分析：基于（样本）协方差矩阵的特征值分解] *输入：*样本集 $D = {bold(x)_1, bold(x)_2, dots, bold(x)_m}^T$；低维空间维数 $d'$.
...
$
  bold(x)_i arrow  bold(x)_i - 1/m sum_(i=1)^m bold(x)_i
$

2. *计算样本协方差矩阵*（提取各维度间的相关性与变异程度.设 $bold(X)$ 为中心化后的 $d times m$ 样本矩阵）：
$
  bold(C) = 1/(m-1) bold(X) bold(X)^T
$
...
#algorithm[样本主成分分析：基于相关矩阵的特征值分解][
  #index[样本主成分分析：基于相关矩阵的特征值分解] 给定样本矩阵 $bold(X)$,利用数据的样本协方差矩阵或者样本相关矩阵的特征值分解进行主成分分析.具体步骤如下：
...
#index[样本主成分分析：基于样本的奇异值分解] *输入*：$m times n$ 样本矩阵 $X$,其每一行元素的均值为零. \
...
== 流形学习#index[流形学习]
流形是在局部与欧氏空间同胚的空间.抛开*微分几何*而直观地说,在一个小邻域里,我们能用欧式距离来刻画.想在局部建立降维映射关系,然后再设法将局部映射关系推广到全局.

#figure(

  align(center,image("/assets/104892ea657562cb985bf412ad9262c9_720.png")),
  caption:[
    一个流形学习的例子.显然我们不希望距离算成绿色的线段,而是根据红色的线走;所以我们需要通过$k$近邻.这就是著名的ISOMAP算法.他通过算$k$近邻来计算距离矩阵,然后再输入到之前说过的MDS降维算法.
  ]
)
=== 局部线性嵌入,LLE#index[局部线性嵌入]
局部线性嵌入试图保持*邻域内*的线性关系,并使得该线性关系在降维后的空间中继续保持.

在原始高维空间里，我们认为每个点 $bold(x)_i$ 都可以被它的邻居们用线性组合完美“拼”出来。为每个样本$bold(x)_i$找到近邻$Q$,然后计算线性重构的系数$bold(w)_i$,也就是
$
  min_(i in cal(I))sum_(i=1)^m ||bold(x)_i - sum_(j in Q_i) w_(i j) bold(x)_j||^2 s.t. sum_(j in Q_i) w_(i j) = 1
$
通过最小化重构误差，把这个权重系数 $w_("ij")$ 解出来：
$
 w_(i j) = (sum_(k in Q_i)C_(j k)^(-1))/(sum_(l,s in Q_i)C_(l s)^(-1)), C_(j k) eq.delta (bold(x_i)-bold(x_j))^T (bold(x_i)-bold(x_k))
$<lle>#note[
  注意$C$*并不是*内积，在几何上是局部协方差矩阵！
]
...
$
#algorithm("ISOMAP")[
  #index[ISOMAP] 样本集$bold(X) = {x_1,dots,x_m}$;
  + 确定$x_i$的k近邻,并且$x_i$与k近邻之间的距离设置为欧式距离.其他的都是$infinity$.
  + 计算$d_(i j)$,填充距离矩阵$bold(D)$;(当然,这里用dij或者floyd等算法了,而不是ML里的算法)
  + return MDS算法的输出
]
#algorithm("LLE")[
#index[LLE] 样本集$bold(X) = {x_1,dots,x_m}$;
  + 确定$x_i$的k近邻.记近邻集合为$Q_i$.
  + 在$Q_i$内算$
 w_(i j) = (sum_(k in Q_i)C_(j k)^(-1))/(sum_(l,s in Q_i)C_(l s)^(-1)), C_(j k) eq.delta (bold(x_i)-bold(x_j))^T (bold(x_i)-bold(x_k))
$.其他的都是0.得到矩阵$bold(W)$.
  + $bold(M) eq.delta (bold(I) - bold(W))^T (bold(I) - bold(W))$.然后,特征值分解$bold(M)$,return *最小*d‘个特征向量构成的矩阵$bold(Z)eq.delta [v_1,dots,v_(d')]$
]
== 度量学习
#definition("度量学习的一般框架")[#index[度量学习]
...
为此将 $W$ 替换为一个普通的*半正定对称矩阵* #mi(`\mathbf{M}`),于是得到了马氏距离（Mahalanobis distance）：#index[马氏距离]
...
对 #mi(`\mathbf{M}`) 进行学习当然要设置一个目标.假定我们是希望提高近邻分类器的性能,则可将 #mi(`\mathbf{M}`) 直接嵌入到近邻分类器的评价指标中去,通过优化该性能指标相应地求得 #mi(`\mathbf{M}`). 
#note[
...
#index[近邻成分分析（NCA）] *输入：* 样本集 #mi(`D = \{(\boldsymbol{x}_1, y_1), (\boldsymbol{x}_2, y_2), \ldots, (\boldsymbol{x}_m, y_m)\}`)；目标维度 $d'$.
...
其中约束 #mi(`\mathbf{M} \succeq 0`) 表明 #mi(`\mathbf{M}`) 必须是半正定的#note[
$v^T M v lt.eq.slant 0 forall v$]. 上式要求在不相似样本间的距离不小于 1 的前提下,使相似样本间的距离尽可能小.
