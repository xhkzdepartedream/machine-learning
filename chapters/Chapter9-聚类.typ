...
$k$均值聚类是基于样本集合划分的聚类算法。$k$均值聚类将样本集合划分为$k$个子集，构成$k$个类，将$n$个样本分到$k$个类中，每个样本到其所属类的中心的距离最小。#note[每个样本只能属于一个类，所以*$k$均值聚类是硬聚类*。下面分别介绍$k$均值聚类的模型、策略、算法，讨论算法的特性及相关问题。]
...
$
  C^* &=arg min_C E_C
$
相似的样本被聚到同类时，损失函数值最小，这个目标函数的最优化能达到聚类的目的。#note[$k$均值聚类的最优解求解是NP困难问题，现实中采用迭代的方法求解。]
#algorithm("K means聚类")[
