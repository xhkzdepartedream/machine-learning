...
== 聚类方法
形式化地：
#note[聚类（Clustering）是按照某个特定标准（如距离）把一个数据集分割成不同的类或簇#index[聚类],使得同一个簇内的数据对象的相似性尽可能大,同时不在同一个簇中的数据对象的差异性也尽可能地大.聚类后同一类的数据尽可能聚集到一起,不同类数据尽量分离.
...
$ a + b + c + d = m(m - 1) / 2 = C_m^2$
#note[即从 $m$ 个样本中任取两个样本的组合数.]

有了$a,b,c,d$我们就可以拼装出不同的评价指标：
#note[
#unim[
...
#definition[
  1. *簇内样本平均距离 #mi(`avg(C)`)*
$ "avg"(C) = (2)/(|C|(|C|-1)) sum_(x_i, x_j in C) "dist"(x_i, x_j) $
...
4. 定义 $mu_i = 1/abs(C_i) sum_(x in C_i) x$ 为簇 $C_i$ 内样本的均值向量,则两个聚类簇 $C_i$ 和 $C_j$ 的*中心距离*为：
...
$ "DI" = min_(1 <= i <= k) { min_(j != i) ( (d_"min" (C_i, C_j)) / (max_(1 <= l <= k) "diam"(C_l)) ) } $
#note[
...
=== $k$均值聚类
#remark[
  随机选一些样本为中心->计算所有样本到各个中心的距离->归到最近的那一类->每一类更新自己的中心。重复。
]
==== $k$均值聚类的算法理念与流程
#note[每个样本只能属于一个类,所以*$k$均值聚类是硬聚类*.下面分别介绍$k$均值聚类的模型、策略、算法,讨论算法的特性及相关问题.]

给定$n$个样本的集合$X={x_1,x_2,dots,x_n}$,每个样本由一个特征向量表示,特征向量的维数是$m$. $k$均值聚类的目标是将$n$个样本分到$k$个不同的类或簇中,这里假设$k<n$. $k$个类$G_1,G_2,dots,G_k$满足$ G_i inter G_j=nothing, union_(i=1)^k G_i=X $. 用$C$表示划分,一个划分对应着一个聚类结果.

策略是通过损失函数的最小化选取最优的划分或函数$C^*$.

首先采用欧氏距离平方作为样本之间的距离$d(x_i,x_j)$, 然后定义*样本与其所属类的中心之间的距离之和*为损失函数,即
$
  E_C=sum_(l=1)^k sum_(C(i)=l) norm(x_i- overline(x)_l)^2
$
式中$overline(x)_l=(overline(x)_(1l),overline(x)_(2l),dots,overline(x)_(m l))^T$是第$l$个类的均值或中心,$n_l=sum_(i=1)^n II(C(i)=l)$, $k$均值聚类就是求解最优化问题：
$
  C^* &=arg min_C E_C
$
相似的样本被聚到同类时,损失函数值最小,这个目标函数的最优化能达到聚类的目的.
#note[$k$均值聚类的最优解求解是NP困难问题,现实中采用迭代的方法求解.]
#algorithm("K means聚类")[
  #index[K means聚类] *输入*: 样本集 $D = (x_1, x_2, dots, x_m)$；聚类簇数 $k$.
...
#figure(image("/fig/类别数与平均直径的关系.png", width: 100%), caption: "类别数与平均直径的关系")
...
#remark[
  ...
  - 得到的类别是*平坦的、非层次化的*.算法是迭代算法,*不能保证得到全局最优*.
...
=== 学习向量量化（LVQ）#index[学习向量量化]

==== 定位与动机
LVQ（Learning Vector Quantization）*假设数据集带有类别标记*,在学习过程中借助这些监督信息,不断*更新原型向量*,从而得到更符合类别边界的划分结果.
...
目标是学得 #mi(`q`) 个 #mi(`n`) 维*原型向量* #mi(`\{\boldsymbol{p}_1, \boldsymbol{p}_2, \ldots, \boldsymbol{p}_q\}`),每个 #mi(`\boldsymbol{p}_i`) 附带一个预设的类别标记 #mi(`t_i \in \mathcal{Y}`). 
#note[
...
==== 完整算法伪代码
#algorithm[高斯混合聚类（GMM）算法][
  #index[高斯混合聚类]
  
  *输入：* 样本集 $D = {x_1, x_2, dots, x_m}$，其中 $x_j in cal(X) subset.eq RR^n$；高斯混合成分个数 $k$. \
  *输出：* 簇划分结果 $C = {C_1, C_2, dots, C_k}$.

  + *初始化高斯混合分布的模型参数*
  
    对 $i = 1, 2, dots, k$，设定初始的三元组参数：$(alpha_i, mu_i, Sigma_i)$
    #note[初始化时，混合系数需满足 $alpha_i > 0$ 且 $sum_(i=1)^k alpha_i = 1$。每一组参数代表一个初始假设的“高斯概率气团”。在工程实践中，通常会先运行一遍 K-means，用其聚类中心和各个簇的协方差来为这些气团进行“垫底”初始化。]

  + *反复迭代更新，直到满足停止条件（EM 算法）*：#note[
    通过“左右互搏”的方式解决参数与成分归属之间的循环依赖：
    #set enum(numbering: "(a)")
    + *E 步（期望步）*：固定当前的气团参数，去计算每个数据点属于各个气团的后验概率 $gamma_(j i)$。这是一种“软分配”，每个点都以不同的概率隶属于所有簇。
    + *M 步（最大化步：均值）*：利用算好的概率作为权重，重新计算气团的中心。对该气团归属感越强的样本，其坐标对新中心 $mu_i'$ 的拉动作用就越大。
    + *M 步（最大化步：协方差）*：基于新的权重分布，更新高斯气团的胖瘦和旋转方向 $Sigma_i'$。
    + *M 步（最大化步：权重）*：看看全场有多少样本实质上归属于该气团，重新对它在整体中所占的资源比例 $alpha_i'$ 进行洗牌。
  ]
    + *[E 步]* 对 $j = 1, 2, dots, m$ 和 $i = 1, 2, dots, k$，计算当前模型下成分 $i$ 生成样本 $x_j$ 的后验概率：
      $ gamma_(j i) = (alpha_i p(x_j | mu_i, Sigma_i)) / (sum_(l=1)^k alpha_l p(x_j | mu_l, Sigma_l)) $
    + *[M 步]* 计算各高斯分量的新均值向量：
      $ mu_i' = (sum_(j=1)^m gamma_(j i) x_j) / (sum_(j=1)^m gamma_(j i)) $
    + *[M 步]* 计算各高斯分量的新协方差矩阵：
      $ Sigma_i' = (sum_(j=1)^m gamma_(j i) (x_j - mu_i)(x_j - mu_i)^T) / (sum_(j=1)^m gamma_(j i)) $
    + *[M 步]* 计算各高斯分量的新混合系数：
      $ alpha_i' = (sum_(j=1)^m gamma_(j i)) / m $
    + *[参数更新]* 将模型参数替换为最新值，若未达到收敛阈值（如对数似然函数的增量极小），则返回步骤 (a) 继续循环：
      $ { (alpha_i, mu_i, Sigma_i) } arrow { (alpha_i', mu_i', Sigma_i') } $

  + *根据最终概率划分确定的簇（一锤定音）*#note()[当高斯气团的位置和形状完全稳定、算法收敛后，我们由“软分配”正式切换为“硬分配”。遍历每个样本，看哪家机器认领它的后验概率最大，它就死心塌地地被归入哪一个簇。]
    初始化各个簇集合 $C_i = emptyset quad (1 <= i <= k)$。对每一个样本 $x_j$，计算其最可能来自的成分标记 $lambda_j$ 并分类：
    $ lambda_j = arg max_(1 <= i <= k) gamma_(j i) $
    $ C_(lambda_j) = C_(lambda_j) union {x_j} $
]
...
层次聚类的树状结构天然适合社区发现任务：通过在不同高度切割树状图,可以得到不同粒度的社区划分. (111)
