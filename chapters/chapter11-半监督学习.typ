...
#index[生成式半监督高斯混合模型 EM 算法] *输入*：有标记样本集 $D_l = {(bold(x)_1, y_1), (bold(x)_2, y_2), dots, (bold(x)_l, y_l)}$； \
...
#algorithm("TSVM 算法")[
  #index[TSVM 算法] *输入*：有标记样本集 $D_l$；未标记样本集 $D_u$；折中参数 $C_l, C_u$. \
...
\tag{13.12}
`)
其中 $bold(L) = bold(D) - bold(W)$ 为拉普拉斯矩阵.注意,这里的$bold(D) eq.delta "diag"{d_i},d_i = sum_(j=1)^n (bold(W))_(i j)$.将函数 $bold(f)$ 划分为有标记部分 $bold(f)_l$ 和未标记部分 $bold(f)_u$.以第 $l$ 行与第 $l$ 列为界,采用分块矩阵表示方式：
$
...
#algorithm("协同训练 (Co-training) 算法")[
  #index[协同训练 (Co-training) 算法] *输入*：有视图标记样本集 $D_l$；未标记样本集 $D_u$；缓冲池大小 $s$；每轮挑选正、反例数 $p, n$；基学习算法 $cal(L)$；训练轮数 $T$. \
  *输出*：视图分类器 $h_1, h_2$. \
  *过程*：
...
#index[约束 k-means 算法] *输入*：样本集 $D = {bold(x)_1, bold(x)_2, dots, bold(x)_m}$； \
...
#index[约束种子 k-means 算法] *输入*：样本集 $D = {bold(x)_1, bold(x)_2, dots, bold(x)_m}$； \
