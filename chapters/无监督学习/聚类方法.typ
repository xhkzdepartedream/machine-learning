...
== 聚类方法
层次聚类又有聚合（自下而上）和分裂（自上而下）两种方法。聚合法开始将每个样本各自分到一个类，之后将相距最近的两类合并，建立一个新的类，重复此操作直到满足停止条件，得到层次化的类别。分裂法开始将所有样本分到一个类，之后将已有类中相距最远的样本分到两个新的类，重复此操作直到满足停止条件，得到层次化的类别。均值聚类是基于中心的聚类方法，通过迭代，将样本分到个类中，使得每个样本与其所属类的中心或均值最近，得到个“平坦的”、非层次化的类别，构成对空间的划分。$k$均值聚类的算法于1967年由MacQueen提出。
...
聚类的核心概念是相似度或距离，有多种相似度或距离的定义。因为相似度直接影响聚类的结果，所以其的选择是聚类的根本问题。具体哪种相似度更合适取决于应用问题的特性。常见的测度有：距离、夹角余弦等。
...
#definition[闵可夫斯基距离][
  ...
  $
  这里$p gt.eq 1$, 当$p=2$时称为欧氏距离，$p=1$时称为曼哈顿距离，$p=infinity$时时切比雪夫距离：
  $
    d_"ij"=max_k |x_"ki"-x_"kj"|
  $
]

马哈拉诺比斯距离（Mahalanobisdistance）简称马氏距离，也是另一种常用的相似度，考虑*各个分量（特征）之间的相关性并与各个分量的尺度无关*。马哈拉诺比斯距离越大，相似度越小；距离越小，相似度越大。因此计算距离时要用方差归一化，这样得出的分量模式与均值模式的距离才具有比较意义。
...
样本的相似度也可以用相关系数(correlation coefficient)来表示。相关系数的绝对值越接近1，表示样本越相似；越接近0，表示样本越不相似。
...
通过聚类得到的类或簇本质是样本的子集。如果一个聚类方法假定一个样本只能属于一个类或类的交集为空集，那么该方法称为硬聚类（hardclustering）方法。否则，如果一个样本可以属于多个类或类的交集不为空集，那么该方法称为软聚类（softclustering）方法。

用$G$表示类或簇(cluster)，用$x_i,x_j$表示类中的样本，用$n_G$表示$G$中样本的个数，用$d_(i j)$表示样本$x_i$与$x_j$之间的距离。类或簇有多种定义，下面给出几个常见的定义。
...
含义：这四个集合的大小之和，等于数据集中所有可能的样本对的总数。数学解释：这是组合数学中的 $C_m^2$，即从 $m$ 个样本中任取两个样本的组合数。
...
#algorithm[
  ...
  *输出*: 样本集合的聚类$C^*$.
  ...
  + 初始化。令$t=0$, 随机选择$k$个样本点作为初始聚类中心$m^((0))=(m_1^((0)), dots,m_l^((0)), dots,m_k^((0)))$.
  + 对样本进行聚类。对固定的类中心$m^((t))=(m_1^((t)), dots,m_l^((t)), dots,m_k^((t)))$, 其中$m^((t))$为类$G_l$的中心，计算每个样本到类中心的距离，将每个样本指派到与其最近的中心的类中，构成聚类结果$C^((t))$.
  + 计算新的类中心。对聚类结果$C^((t))$，计算当前各个类中的样本的均值，作为新的类中心$m^((t+1))=(m_1^((t+1)), dots,m_l^((t+1)), dots,m_k^((t+1)))$.
  + 如果迭代收敛或符合停止条件，输出$C^*=C^((t))$；否则，令$t=t+1$, 返回步骤2.
]
...
聚类结果的质量可以用类的平均直径来衡量。一般地，类别数变小时，平均直径会增加; 类别数变大超过某个值以后，平均直径会不变，而这个值正是最优的，值。下图说明类别数与平均直径的关系。实验时，可以采用二分查找，快速找到最优的值。
