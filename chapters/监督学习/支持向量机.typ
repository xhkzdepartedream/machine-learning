...
== 支持向量机
支持向量机(support vector machine, SVM)是一种二类分类模型，它的基本模型是定义在特征空间上的间隔最大的线性分类器，间隔最大使他有别于感知机；支持向量机还包括核技巧，这使它成为实质上的非线性分类器。支持向量机的学习策略就是间隔最大化，可形式化为一个求解凸二次规划的问题，也等价于正则化的合页损失函数的最小化问题。支持向量机的学习算法是求解凸二次规划的最优化算法。

由简到繁的模型有：线性可分支持向量机(linear SVM in linearly separable case)、线性支持向量机(linear SVM) 以及非线性支持向量机(non-linear SVM)。简单模型是复杂模型的基础，也是复杂模型的特殊情况。当*训练数据线性可分*时，通过硬间隔最大化，学习一个线性可分支持向量机，又称*硬间隔支持向量机*；当*训练数据近似线性可分*时，通过软间隔最大化，也学习一个线性的分类器，即*线性支持向量机*，又称为*软间隔支持向量机*；当*训练数据线性不可分*时，通过使用*核技巧及软间隔最大化*，学习*非线性支持向量机*。
...
=== 线性可分支持向量机与硬间隔最大化
#abstract[
因为过去感知机只是为了能够找到正确划分的超平面而已，这会有无数个而且对于新样本的划分质量不高，所以评判的标准从*正确划分训练集*变成了*以最大把握划分训练集*，因而提出了*函数间隔和几何间隔*的概念。进一步地，由于间隔值取值本身不影响最优化问题的答案，所以最后得到的优化问题是：
$
  &min_(w,b) 1/2 norm(w)^2\
  &s.t. quad y_i (w dot x_i+b) - 1 gt.eq.slant 0, #h(1em)i=1,2,dots,N
$
由此得到分离超平面：
  $
    w^* dot x+b^*=0
...
==== 函数间隔和几何间隔
一般来说，一个点距离分离超平面的远近可以表示分类预测的确信程度。在超平面$w dot x+b=0$确定的情况下，$|w dot x+b|$能够相对地表示点$x$距离超平面的远近。而$w dot x+b$的符号和类标记$y$的符号是否一致能够表示分类是否正确，所以可用量$y(w dot x+b)$来表示分类的正确性及确信度，这就是*函数间隔(functional margin)*的概念。
...
选择超平面时，只有函数间隔还不够。因为只要成比例地改变$w$和$b$。超平面并不会改变，而函数间隔却变成了两倍，因而要对$w,b$进行一定地规范化，这时函数间隔就变成了几何间隔(geometric margin)。

#definition[
  对于给定训练数据集$T$和超平面$(w,b)$，定义超平面$(w,b)$关于样本点的几何间隔为
  $
...
==== 间隔最大化
间隔最大化的直观解释是：对训练数据集找到几何间隔最大的超平面意味着以充分大的确信度对训练数据进行分类。也就是说，不仅将正负实例点分开，而且对最难分的实例点（离超平面最近的点）也有足够大的确信度将他们分开。这样的超平面对于未知新实例有很好分类预测能力。
...
而函数间隔$hat(gamma)$的取值其实对上面的不等式约束是没有影响的（与$w,b$在不等式中都是一次的），所以可用取$hat(gamma)=1$，而如果最大化$1/norm(w)$，其实和最小化$1/2 norm(w)^2$是等价的，所以问题转换为了
$
  &min_(w,b) 1/2 norm(w)^2\
...
#definition[
  在线性可分情况下，训练数据集的样本点中与分离超平面距离最近的样本点的实例称为*支持向量(support vector)*. 支持向量是使约束条件式等号成立的点，即：
  $
    y_i (w dot x_i + b)-1=0
  $
  ...
  $
    H_2:w dot x+b=-1
  $
  上。在$H_1$和$H_2$上的点就是*支持向量*。
]
#remark[
  其实就是卡住最大间隔超平面的那些点，被称为支持向量。
]
...
$
  L(w,b,alpha)=1/2 norm(w)^2-sum_(i=1)^N alpha_i y_i (w dot x_i + b)+sum_(i=1)^N alpha_i
$
...
将$L(w,b,alpha)$分别对$w,b$求偏导数并令其等于0：
$
...
即
$
  min_(w,b) L(w,b,alpha)=-1/2  sum_(i=1)^N sum_(j=1)^N alpha_i alpha_j y_i y_j (x_i dot x_j)+sum_(i=1)^N alpha_i
$
现在对偶问题就转换为了
$
...
$
  T={(x_1,y_1),(x_2,y_2),dots,(x_N,y_N)}
$
其中，$x_i in cal(X)=R^n, y_i in cal(Y)={+1,-1},i=1,2,dots,N$. $x_i$为第$i$个特征向量，也称为实例，$y_i$为$x_i$的类标记。当$y_i = +1$时，称$x_i$为正例；否则称为负例。$(x_i,y_i)$称为样本点。假设训练数据集不是线性可分的，通常训练数据中有一些特异点，将这些特异点除去后，剩下大部分样本点组成的集合是线性可分的。

线性不可分意味着某些样本点$(x_i,y_i)$不能满足函数间隔大于等于1的约束条件。为了解决这个问题，可以对每个样本点$(x_i,y_i)$引入一个松弛变量$xi_i gt.eq.slant 0$，使函数间隔加上松弛变量大于等于1.这样，约束条件变为
$
...
这里，$C>0$称为惩罚参数，一般由应用问题决定，$C$值大时对误分类的惩罚增大，$C$值小时对误分类的惩罚减小。最小化目标函数有两层含义：使$1/2 norm(w)^2$尽量小即间隔尽量大，同时使误分类点的个数尽量少，$C$是调和二者的系数。
...
根据拉格朗日对偶性（对于 $g(x) gt.eq.slant 0$ 的约束，构建 $L = f(x) - sum lambda g(x)$），我们在目标函数中减去乘子与约束的乘积：
  $
    ...
    - sum_(i=1)^N mu_i underbrace(xi_i, "非负约束")
  $
整理得到原始最优化问题的拉格朗日函数
$
  L(w,b,xi,alpha,mu)=1/2 norm(w)^2+C sum_(i=1)^N xi_i-sum_(i=1)^N alpha_i [y_i (w dot x_i + b)-1+xi_i]-sum_(i=1)^N mu_i xi_i
$
其中，$alpha_i gt.eq.slant 0, mu_i gt.eq.slant 0$
...
=== 非线性支持向量机与核函数
非线性分类问题是指通过利用非线性模型才能很好地进行分类的问题。一般来说，对给定的一个训练数据集$T={(x_1,y_1),(x_2,y_2),dots,(x_N,y_N)}$，其中，实例$x_i$属于输入空间，$x_i in cal(X)=R^n$,对应的标记有两类$y_i in cal(Y)={-1,+1}, i=1,2,dots,N$。如果能用$R^n$中的一个超曲面将正负例正确分开，则称这个问题为非线性可分问题。
...
#remark[
  核技巧属于这种方法。通过一个非线性变换将输入空间（欧氏空间$R^n$或离散集合）对应于一个特征空间（希尔伯特空间$cal(H)$），使得在输入空间$R^n$中的超曲面模型对应于特征空间$cal(H)$中的超平面模型（支持向量机）。
]
...
假设$K(x,z)$是定义在$cal(X) times cal(X)$上的对称函数，并且对任意的$x_1,x_2,dots,x_m in cal(X)$，$K(x,z)$关于$x_1,x_2,dots,x_m$的Gram矩阵是半正定的。可以依据函数$K(x,z)$构成一个希尔伯特空间，步骤是：首先定义映射$phi.alt$并构成向量空间$cal(S)$，然后在$cal(S)$上定义内积构成内积空间，最后将$cal(S)$完备化构成希尔伯特空间。

#remark[
  Gram矩阵是由核函数$K(x,z)$在给定数据集上计算得到的矩阵。具体来说，假设有$m$个数据点$x_1,x_2,dots,x_m in cal(X)$，则Gram矩阵$G$是一个$m times m$的矩阵，其中元素$G_("ij")=K(x_i,x_j)$
]
...
$
  f(x)="sign"[sum_(i=1)^(N_s) a_i^* y_i (x_i dot x+1)^p+b^*]
$

*2. 高斯核函数(Gaussian kernel function)*
$
  K(x,z)=exp(-frac(norm(x-z)^2,2 sigma^2))
$
对应的支持向量机是高斯径向基函数(radial basis function)分类器。在此情形下，分类决策函数成为
$
  f(x)="sign"[sum_(i=1)^(N_s) a_i^* y_i exp(-frac(norm(x-z)^2,2 sigma^2))+b^*]
$
*3. 字符串核函数*
