...
$#remark[
  这就是说，分类决策函数只依赖于输入$x$和所有训练样本输入的内积。
]
...
==== 线性支持向量机
通常训练数据集不是线性可分的，意味着某些样本点$(x_i,y_i)$不能满足函数间隔大于等于1的约束条件。

为了解决这个问题，可以对每个样本点$(x_i,y_i)$引入一个松弛变量$xi_i gt.eq.slant 0$，使函数间隔加上松弛变量大于等于1.这样，约束条件变为
$
