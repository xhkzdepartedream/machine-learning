...
== 杂项
=== 说出五种防止过拟合的办法
- 正则化(Regularization)：在损失函数中加入惩罚项(L1 / L2),限制模型参数的量级,使模型不能随意拟合噪声.L2正则化(Ridge)使参数趋近于零,L1(Lasso)还有特征选择的效果.
- Dropout：在神经网络训练时随机"关闭"一部分神经元,防止神经元之间产生过度依赖,迫使网络学习更鲁棒的特征表示.这是深度学习中最常用的正则化手段之一.
- Early Stopping(早停法)：在训练过程中持续监测验证集误差,一旦验证集误差不再下降甚至开始回升,就提前停止训练,避免模型在训练集上过度拟合.
- 数据增强(Data Augmentation)：通过对训练数据做随机变换(翻转、裁剪、旋转、加噪声等),人为扩充训练集规模,使模型接触到更多样的样本,从而提高泛化能力.
- 减小模型复杂度(Model Simplification)：直接缩减模型的容量——减少神经网络层数/神经元数、降低多项式阶数、剪枝决策树等.从根源上限制模型拟合噪声的能力,是最直接的方法
